...
## 演習の目的
- ここでは、事前に用意してある Python スクリプトを実行し、前のステップで編集したローカルのデータを元に、既存の ArcGIS Online 上のホスト フィーチャ レイヤーをアップデートします。演習を通じて次の事項について習得・確認していただきます。
  - Python コマンド プロンプトの使用方法 (Python 環境の一覧取得・切り替え、カレント ディレクトリの移動、スクリプトの実行)
  - Python スクリプトによるホスト フィーチャ レイヤーの更新
  - ホスト フィーチャ レイヤーを参照している Web マップ上でデータが更新されていることを確認
...
1. ディレクトリを移動したら src ディレクトリ内にある web_map_sync_main.py ファイルを実行しましょう。
    ...
    実行完了までしばらく時間がかかるので、余裕があればテキストエディタ等で EJWater\script\src ディレクトリ内の web_map_sync.py ファイルを開き、ソースコードを確認してみましょう。
...
## まとめ
この演習では Python コマンド プロンプトの基本的な使用方法、Python スクリプトの実行などを行いました。また、ホスト フィーチャ レイヤーが更新されていることを Web マップで確認しました。

タスクスケジューラなどを使うことでPython スクリプトの実行を自動で定期的に行うことでニア リアルタイムで参照するデータを最新状態に保つことが可能です。タスク スケジューラの設定方法は[ArcGIS for Developer 開発リソース集](http://esrijapan.github.io/arcgis-dev-resources/tips/python/automate-tasks/)にも詳しく記載しておりますので、ご覧ください。

また、Web マップは ArcGIS プラットフォーム上の様々なアプリケーションで参照することが可能です。次の ArcGIS API for JavaScript のハンズオンでも同じ Web マップを参照しますので、ハンズオンの最後に完成した Web アプリからも、今回更新したデータが反映されていることを確認してみてください。

...
{
    class ConnectivityModeOfflineToBoolConverter : IValueConverter
    {
        /// <summary>
        ...
        object IValueConverter.Convert(object value, Type targetType, object parameter, CustomCultureInfo culture)
        {
            if (value is ConnectivityMode)
            {
                ...
                if (parameter != null && parameter.ToString() == "Inverse")
                {
                    ...
                    return ((ConnectivityMode)value == ConnectivityMode.Online) ? true : false;
                }
                else
                {
                    //if value is ConnectivityMode.Offline, visibility is collapsed 
                    return ((ConnectivityMode)value == ConnectivityMode.Online) ? false : true;
                }
            }
            else
                return false;
        ...
        object IValueConverter.ConvertBack(object value, Type targetType, object parameter, CustomCultureInfo culture)
        {
            if (value is bool)
            {
                ...
                if (parameter != null && parameter.ToString() == "Inverse")
                {
                    ...
                    return ((bool)value == false) ? ConnectivityMode.Offline : ConnectivityMode.Online;
                }
                else
                {
                    //if visibility is collapsed return ConnectivityMode.Offline, otherwise ConnectivityMode.Online
                    return ((bool)value != true) ? ConnectivityMode.Online : ConnectivityMode.Offline;
                }
            }
            else
                return false;

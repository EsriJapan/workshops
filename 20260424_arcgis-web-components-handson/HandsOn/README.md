...
ArcGIS Maps SDK for JavaScript では、`<script type="module" src="https://js.arcgis.com/5.0/"></script>` のタグを 1 つ追加することで、 `ArcGIS Maps SDK for JavaScript` の `Core API` および `マップ コンポーネント` をはじめ各種コンポーネント、`Calcite Design System` のライブラリーを追加することができます。
...
```html
  <body>
    ...
    <!-- 手順 2-1 Map コンポーネントで topo-vector ベースマップで日本を中心で表示。また、マップ上に拡大、縮小ボタンを表示 End -->
    <script></script>
  </body>
```
...
```html
    <script type="module">
      ...
      const layer = new FeatureLayer({
          url: "https://services.arcgis.com/wlVTGRSYTzAbjjiC/arcgis/rest/services/hinanbasho/FeatureServer/0",
          outFields: ["*"]
      });
      ...
      arcgisMap.addEventListener("arcgisViewClick", (evt) => {
          ...
          const layerView = arcgisMap.layerViews.find(lv => {
            return lv.layer.title === layer.title
          });
          ...
          layerView.queryFeatures(query)
              .then(result => {
                  // 検索結果をアラートとして表示
                  alert(result.features[0].attributes["施設_場所名"]);
              });
      });
...
```html
  ...
  <!-- 手順 4-1-1 Calcite Design System の Shell コンポーネントを追加 End -->
    ...
    <calcite-shell-panel slot="panel-end" slot="end" display-mode="float-content" collapsed>
...
3. `<script>` タグ内で`手順 3-2` に記述したコードの下に以下のコードを追記します。
```html
    <script type="module">
      // ～～ 中略 ～～
      });
...
```
ブラウザーを更新すると以下のようにヘッダー部分の背景色が変更されているのが確認できます。

![オプション 1 : ヘッダー デザインの変更](./img/option.png)

ナビゲーション コンポーネントの要素に設定されている [CSS 変数](https://developers.arcgis.com/calcite-design-system/components/navigation/#styles)を用いることでコンポーネントのデザインを変更することができます。このように Calcite コンポーネントのデザインを CSS 変数を用いることで変更することができます。
...
<html lang="ja">
  ...
  <body>
    ...
    <script type="module">
      ...
      arcgisMap.addEventListener("arcgisViewClick", (evt) => {
          // 追加した FeatureLayer のレイヤー ビューを取得
          let layerView = arcgisMap.layerViews.find(lv => {
            return lv.layer.title === layer.title
          });
          // クリックした地点のフィーチャを空間検索するための条件を設定
          const query = layer.createQuery();
